  ...
  <el-col :span="8">
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <h5>库存统计</h5>
        <p class="ct-title-text">统计当前时间点库内卷烟【可用总库存数量】及【入库数量】、【出库数量】</p>
      </div>
      ...
      this._ajax({url: this.rootAPI + 'home/StockwarningRank?day='+day})
        .then((function(d) {
            if(d.state === 0) {
              var xRow = [];
              var one = [],two = [],three = [];
              d.aaData.one.forEach(temp=>{
                xRow.push(temp.days.replace(" 00:00:00",""))
     ...
     this._ajax({url: this.rootAPI + 'home/queryHome'})
        .then((function(d) {
            if(d.state === 0) {
              ...
              this.ctD.nil = this.ctC.count - this.ctC.ok;
              ...
              this.CBChars.setOption({
                series: [
                    {
                        data:[
                            {value:kb, name:'占用数量',itemStyle:{
                              color:"#1E88E5"
                            }},
                            {value:ka, name:'闲置数量',itemStyle:{
                              color:"#26C6DA"
                            }}
                        ]
                    }
          ...
          series: [
              {
                  ...
                  data:[
                      {value:335, name:'占用数量',itemStyle:{
                        color:"#1E88E5"
                      }},
                      {value:310, name:'闲置数量',itemStyle:{
                        color:"#26C6DA"
                      }}
                  ]
              }
