		...
		<el-card class="box-card" style="margin-right: 5px; margin-left: 5px;" shadow="hover">
			...
			<div>
				<el-form class="modal-form" :inline="true" :model="form" :rules="rules" ref="form" label-width="120px">
						...
						<el-col :span="6">
							<el-form-item label="品牌" prop='brand'>
								<selectInput :value.sync="form.brand" filterable>
									<el-option v-for="item in brandOption" :key="item.key" :label="item.value" :value="item.key">
									</el-option>
						...
						<el-col :span="6">
							<el-form-item label="显示标签" prop='intB'>
								<selectInput :value.sync="form.intB">
									<el-option v-for="item in intBOption" :key="item.key" :label="item.value" :value="item.key">
									</el-option>
								</selectInput>
							</el-form-item>
						</el-col>
						...
						<el-col :span="6">
							<el-form-item label="是否上架" prop='transactionStatu'>
								<selectInput :value.sync="form.transactionStatu">
									<el-option v-for="item in transactionStatuOption" :key="item.key" :label="item.value" :value="item.key">
									</el-option>
								</selectInput>
							</el-form-item>
						</el-col>
					...
					<el-row>
						...
						<el-col :span="6">
							<el-form-item label="是否特价" prop='offerStatue'>
								<selectInput :value.sync="form.offerStatue">
									<el-option v-for="item in offerStatueOption" :key="item.key" :label="item.value" :value="item.key">
									</el-option>
								</selectInput>
							</el-form-item>
						</el-col>
						<el-col :span="6">
							<el-form-item label="截止日期" prop='endTime'>
								<dateEditorDaterange :dateValue.sync="form.endTime" :showPickerOptions="false" type="date" style="width: 200px;"></dateEditorDaterange>
							</el-form-item>
						</el-col>
						<el-col :span="6">
					...
					<el-row>
						<el-col :span="6">
							<el-form-item label="首图片" prop='topImg'>
								<el-upload class="avatar-uploader" :action="uploadURL" :show-file-list="false" :on-success="topSuccess" accept="image/*"
						...
						<el-col :span="6">
							<el-form-item label="过户费" prop='transferPriceState'>
								<selectInput :value.sync="form.transferPriceState">
									<el-option v-for="item in transferPriceStateOption" :key="item.key" :label="item.value" :value="item.key">
									</el-option>
								</selectInput>
							</el-form-item>
						</el-col>
					...
					<el-row>
						<el-col :span="6">
							<el-form-item label="上牌日期" prop='upbkTime'>
								<dateEditorDaterange :dateValue.sync="form.upbkTime" :showPickerOptions="false" type="date" style="width: 200px;"></dateEditorDaterange>
							</el-form-item>
						</el-col>
						<el-col :span="6">
							<el-form-item label="档位" prop='dw'>
								<selectInput :value.sync="form.dw">
									<el-option v-for="item in dwOption" :key="item.key" :label="item.value" :value="item.key">
									</el-option>
								</selectInput>
							</el-form-item>
						</el-col>
					...
					<el-row>
						<el-col :span="24">
							<el-upload accept="image/*" :action="uploadDetailURL" list-type="picture-card" :file-list="detailImg"
							 :on-success="detailSuccess" :before-upload="beforeDetailUpload" :on-remove="onRemoveDetail">
					...
					success: function(response) {
						if (response.status == 200 && response.list.length > 0) {
							...
							response.list.forEach(temp => {
								_this.brandOption.push({
									key: temp.makeName,
									value: temp.groupName + " " + temp.makeName
								})
							})
						}
					},
			...
			topBeforeUpload(file) {
				return new Promise((resolve, reject) => {
					this.$nextTick(() => {
						...
						//图片限制4M，其他附件限制50M
						let limitType = 10
						this.uploadURL = configs.uploadFileURL + '/file/imgUploadFirst?savePath=car&token=' + local.get('token')
						const isLt = file.size / 1024 / 1024 < limitType;
						if (!isLt) {
							this.$message.error('上传文件大小不能超过 ' + limitType + 'MB!');
							reject()
			...
			doFileSet(data) {
				...
				if (data.detailImgObj) {
					...
					data.detailImgObj.forEach(temp => {
						_this.detailImg.push({
							loadId: temp.id,
							url: imgYU + temp.filePath
						})
					})
				}
			},
