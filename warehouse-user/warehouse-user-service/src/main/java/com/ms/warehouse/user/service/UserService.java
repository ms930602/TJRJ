	...
	public ListRespVO list(ListReqVO<UserEntity> reqVO) {
		...
		if (!org.apache.commons.lang.StringUtils.isBlank(reqVO.getExtend()) && reqVO.getExtend().equals("true")) {
			...
			for (UserEntity userEntity : userList) {
					...
					params.put("PloginName", userEntity.getLoginName());//登录名
					com.ms.warehouse.manage.entity.PersoninfoEntity personinfoEntity=personinfoService.queryByMapParams(params);//返回值结果对象
					//校验对象是否存在
				// 校验对象是否存在
	...
	public BaseRespVO delete(@Param("userId") Long[] userId) {
		for (Long long1 : userId) {
			...
			if (entity != null) {
				...
				com.ms.warehouse.manage.entity.PersoninfoEntity personinfoEntity = personinfoService
						.queryByMapParams(params);// 返回值
				if (personinfoEntity != null) {
