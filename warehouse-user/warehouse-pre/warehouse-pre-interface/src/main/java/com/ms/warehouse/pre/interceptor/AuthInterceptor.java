package com.ms.warehouse.pre.interceptor;
...
import com.ms.warehouse.pre.utils.Constants;
...
public class AuthInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		SessionUser loginUser = (SessionUser) session.getAttribute(com.ms.warehouse.common.Constants.SESSION_LOGIN_USER);
		if (loginUser == null) {
		...
		if (!reqIp.startsWith("0:0:0:0") && !"127.0.0.1".equals(reqIp)
		        && !reqIp.equalsIgnoreCase(session.getAttribute(com.ms.warehouse.common.Constants.REQUEST_CLIENT_IP) + "")) {
			logger.error("访问IP与登录不一致！{}---{}", reqIp, session.getAttribute(com.ms.warehouse.common.Constants.REQUEST_CLIENT_IP));
